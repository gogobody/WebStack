...
<?php if (count((explode("\r\n", ($this->options->JIndexRecommend)))) == 4 || $this->options->JIndexCarousel) : ?>
    <div class="index-banner">
        <?php if ($this->options->JIndexCarousel) : ?>
            ...
            ?>
            <div id="carouselSwiperCaptions" class="carousel slide" data-ride="">
                ...
                <div class="carousel-inner">
                    <?php for ($i = 0; $i < $long; $i++) {
                        $img = trim(explode("||", $string_arr[$i])[0]);
                        $url = trim(explode("||", $string_arr[$i])[1]);
                        $title = trim(explode("||", $string_arr[$i])[2]);
                    ?>
        ...
        <?php
        $recommend = trim($this->options->JIndexRecommend);
        $recommend_arr = explode("\r\n", $recommend);
        $number = count($recommend_arr);
        if ($number === 4) {
        ?>
            <div class="recommend <?php if (!$this->options->JIndexCarousel) : ?>noCarousel<?php endif; ?>" id="recommend">
                <?php for ($i = 0; $i < $number; $i++) {
                    $img = trim(explode("||", $recommend_arr[$i])[0]);
                    $url = trim(explode("||", $recommend_arr[$i])[1]);
                    $title = trim(explode("||", $recommend_arr[$i])[2]);
                ?>
                    <a title="<?php _e($title); ?>" href="<?php _e($url); ?>" target="_blank">
                        <img class="lazyload" src="<?php echo GetLazyLoad() ?>" data-src="<?php _e($img); ?>">
                        <div class="desc">
